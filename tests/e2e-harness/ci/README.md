...
`audio-endurance.yml` provides **manual-only** qualification for `s13-coded-endurance` on the same three native operating systems. It supports direct manual dispatch and a reusable entry called only when the existing reliability workflow is manually dispatched with `run_endurance=true`. That boolean defaults to false; push/pull-request runs and ordinary manual runs retain the short suites. An endurance dispatch skips the short suites and runs only the three endurance jobs. It forces a real 18,900-second (5h15) capture on Windows and Apple Silicon and an 18,300-second (5h05) capture on Intel, whose five-hour finalization alone takes about 18 minutes, with ordinary microphone processing and the default 17,700-second (4h55) source rotation. It exposes no input that can shorten capture or disable processing. The supervisor rejects altered duration/processing/rotation settings, strips credentials, supplies the checked-out Git SHA to the harness, and requires `fiveHourQualificationPassed: true` in the result before declaring success. A local 45-second smoke cannot satisfy this workflow.

The endurance supervisor allows the requested capture plus 40 minutes, capped at 350 minutes (345 for 5h05, 350 for 5h15); the capture step allows 350 or 353 minutes and the native job 360. This gives 35 to 40 minutes beyond the requested capture for startup, reference generation, finalization, local upload, and full decoded verification, with about seven minutes of job time left for dependency installation, build, and artifact upload. The earlier 25-minute remainder expired on the Intel runner at 93.8 % of the five-hour encode (run 33988551101; the later replay of the same source completed normally), so the budget was raised; the 18,300-second capture and every oracle tolerance are unchanged. A slow or terminated job fails and retains whatever evidence the runner can upload; elapsed capture time alone does not count as a pass. No timers or audio are accelerated. The scenario writes its numbered input WAV at 16 kHz (about 0.6 GB for 5h15; the coded symbols lie below 3.7 kHz), because the fake microphone reads the whole file into memory before delivering audio and the former 1.8 GB 48 kHz file blocked `getUserMedia` for 11.9–12.6 s on the macOS runners (run 34697743281); the ten-second acquisition bound is unchanged. Native finalization anchors the microphone at its recorder start call, so the final's expected duration runs from that call to the stop, and its trailing silence may equal the measured start-call-to-first-audio delay, which is reported and bounded at ten seconds. Run `run_failure_diagnostics=true` with `failure_diagnostic_scope=startup` and `startup_sample_rate=16000` to measure acquisition for the 16 kHz fixture size on all three runners in about 25 minutes. It checks available disk space before generating the input WAV, samples free space and recording/persistence/memory state every 30 seconds, writes progress JSONL, and preserves a bounded summary. A critical low-space condition stops only the synthetic app and retains its durable sources.
