      ...
      <q-toolbar class="compact-toolbar">
        ...
        >
          ...
          <div
            ...
            class="minutes-badge"
            :class="{ 'unlimited': minutesStore.unlimited, 'low': !minutesStore.unlimited && minutesStore.remainingMinutes <= 30 && minutesStore.remainingMinutes > 0, 'empty': !minutesStore.unlimited && minutesStore.remainingMinutes <= 0 }"
          >
            <q-icon
              :name="minutesStore.unlimited ? 'all_inclusive' : 'schedule'"
              size="14px"
            />
            <span v-if="minutesStore.unlimited">Unlimited</span>
            <span v-else>{{ Math.round(minutesStore.remainingMinutes) }} Min.</span>
            <q-tooltip>
              <span v-if="minutesStore.unlimited">Unlimited recording minutes</span>
              <span v-else>{{ Math.round(minutesStore.remainingMinutes) }} of {{ Math.round(minutesStore.totalMinutes) }} minutes remaining</span>
            </q-tooltip>
          </div>
...
// Fetch minutes on mount and start auto-refresh
onMounted(async () => {
