    ...
    >
      <q-toolbar class="compact-toolbar">
        ...
        <div
          ...
          class="header-right"
        >
          <!-- Minutes/Credits Badge -->
          <div
            v-if="authStore.minutes"
            class="minutes-badge"
            :class="{ 'unlimited': authStore.minutes.unlimited, 'low': !authStore.minutes.unlimited && authStore.minutes.remaining <= 30 && authStore.minutes.remaining > 0, 'empty': !authStore.minutes.unlimited && authStore.minutes.remaining <= 0 }"
          >
            <q-icon
              :name="authStore.minutes.unlimited ? 'all_inclusive' : 'schedule'"
              size="14px"
            />
            <span v-if="authStore.minutes.unlimited">Unlimited</span>
            <span v-else>{{ Math.round(authStore.minutes.remaining) }} min</span>
            <q-tooltip>
              <span v-if="authStore.minutes.unlimited">Unlimited recording minutes</span>
              <span v-else>{{ Math.round(authStore.minutes.remaining) }} of {{ Math.round(authStore.minutes.total) }} minutes remaining</span>
            </q-tooltip>
          </div>
