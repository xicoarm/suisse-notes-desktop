...
2. **No end-to-end storage proof:** status confirms a Meeting row under the current backend contract. Add server-side blob existence/length/checksum verification and persistent upload receipts before claiming byte-for-byte durable remote custody. The read-only Azure check found StorageV2 with Standard_LRS and 30-day blob soft deletion. Container recovery, versioning and restore policy were not returned by that service API and remain unverified; no restore drill was performed.
