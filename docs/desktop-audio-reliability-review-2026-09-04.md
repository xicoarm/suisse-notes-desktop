...
| High | Metadata updates could erase processing warnings. | Preserve main-process capture warnings when renderer metadata is updated. Warnings prevent automatic source deletion. |
...
- Bind receipts to owner and the source file size/mtime captured before transfer. Refuse deletion if the file or preserved recording sources change after confirmation. A changed source requires review instead of being silently deduplicated against the wrong content. Initial receipts explicitly deny deletion until confirmation finishes.
...
2. **No end-to-end storage proof:** status confirms a Meeting row under the current backend contract. Add server-side blob existence/length/checksum verification and persistent upload receipts before claiming byte-for-byte durable remote custody. Azure redundancy, soft delete/versioning, retention and restore tests were not verified in this desktop review.
